//Quero criar essa nova feature, que deve aparecer em hub/manager/users/userId (na tabela 'Plano de Aula')

//Estrutura de dados dos planos de aula. Tanto os modelos prontos como os personalizados.
//Os planos de aula são conjuntos de lessons
//Os planos de aula devem ser conectados as aulas marcadas do aluno para o período
//Primeiro uma função pega todas as aulas marcadas do aluno para o período em ordem cronológica
//O manager vai escolher então as lessons que serão usadas no plano de aula até completar a quantidade certa (levando em conta a quantidade de aulas), ou vai escolher um plano de aulas pronto.
//E por fim uma função que conecta as lessons do plano de aula com as aulas marcadas do aluno para o período na ordem correta
...
export interface Plan {
  ...
  lessons: Array<{
    ...
    learningItemsIds: Array<{id: string, srsData?: SRSData; /*Optional, initializes if missing*/ updatedAt: any; lastReviewedAt?: Date | string;}>; // IDs dos Learning Items (Vocabulário) associados à Lesson. No final de cada semana, esse item precisa estar vazio porque os componentes foram enviados para a lista correta dependento do estado (aprendido ou revisar)
    learningStructureIds: Array<{id: string, srsData?: SRSData; /*Optional, initializes if missing*/ updatedAt: any; lastReviewedAt?: Date | string;}>; // IDs dos Learning Structures (Gramática) associados à Lesson. No final de cada semana, esse item precisa estar vazio porque os componentes foram enviados para a lista correta dependento do estado (aprendido ou revisar)
    }>;
    ...
    learnedComponentsIds: Array<{id: string, srsData?: SRSData; /*Optional, initializes if missing*/ updatedAt: any; lastReviewedAt?: Date | string;}>;// IDs dos Learned Components (Itens and Structures) associados à Lesson (vamos criar uma função para isso, para definir quando algo foi aprendido)
    ...
    reviewLearnedComponentsIds: Array<{id: string, srsData?: SRSData; /*Optional, initializes if missing*/ updatedAt: any; lastReviewedAt?: Date | string;}>; // IDs dos Learned Components (Itens and Structures) associados à Lesson que serão revisados depois de um tempo aprendidos
