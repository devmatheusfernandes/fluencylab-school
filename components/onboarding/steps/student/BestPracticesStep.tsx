...
import { Clock, CalendarX, Wallet, AlertTriangle, ShieldCheck } from "lucide-react";
...
export const BestPracticesStep: React.FC<OnboardingStepProps> = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto max-h-[60vh] overflow-auto">
      ...
      <div className="grid grid-cols-1 gap-4">
        {rules.map((rule, idx) => (
          <Card key={idx} className="p-4 flex gap-4 items-start border-l-4 border-l-primary/20 hover:border-l-primary transition-all">
            <div className="mt-1 flex-shrink-0 bg-gray-50 dark:bg-gray-800 p-2 rounded-lg">
                {rule.icon}
            </div>
            <div>
              <Text className="font-bold text-gray-900 dark:text-gray-100">{rule.title}</Text>
              <Text size="sm" className="text-gray-600 dark:text-gray-400 leading-relaxed mt-1">
                {rule.text}
              </Text>
            </div>
          </Card>
