    ...
    <div className="p-4 md:p-8 max-w-xl mx-auto text-center">
      {status === "verified" ? (
        <Card className="p-8 bg-green-50 dark:bg-green-900/10 border-green-200">
          ...
          <Text size="sm" className="text-green-600 mt-2">Redirecionando...</Text>
        </Card>
      ) : (
        <div className="space-y-6">
          ...
          <div>
            ...
            <Text className="text-gray-500 mt-2">
              Para ativar sua conta de professor, precisamos confirmar o email: <br/>
              <strong>{session?.user?.email}</strong>
            </Text>
          </div>

          <div className="flex flex-col gap-3">
            <Button onClick={resendEmail} disabled={loading} variant="outline" className="w-full">
              <Mail className="w-4 h-4 mr-2" /> Reenviar link de verificação
            </Button>
            <Button onClick={checkStatus} variant="ghost" className="w-full text-sm">
