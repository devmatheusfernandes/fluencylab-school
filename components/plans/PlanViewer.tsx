...
export function PlanViewer({ plan, totalScheduledClasses }: PlanViewerProps) {
  ...
  const t = useTranslations("Plans.viewer");

  const scheduledLessonsCount = plan.lessons.filter(
    (l) => l.scheduledClassId,
  ).length;
  const progressPercentage =
    totalScheduledClasses && totalScheduledClasses > 0
      ...
      {totalScheduledClasses !== undefined && (
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t("coverageTitle")}</span>
            <span className="font-medium">
              {t("plannedClasses", {
                planned: scheduledLessonsCount,
                total: totalScheduledClasses,
              })}
            </span>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            {scheduledLessonsCount < totalScheduledClasses
              ? t("remainingClasses", {
                  count: totalScheduledClasses - scheduledLessonsCount,
                })
              : t("allPlanned")}
          </p>
        </div>
