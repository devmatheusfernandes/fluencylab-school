...
export function TaskKanbanView({ tasks, onTaskClick, onStatusChange, isLoading }: TaskKanbanViewProps) {
...
function KanbanColumn({ id, title, color, tasks, users, onTaskClick, onMoveStatus, isFirst, isLast }: KanbanColumnProps) {
  ...
  return (
    <div ref={setNodeRef} className={cn(
        "flex-1 min-w-[300px] w-[300px] rounded-xl flex flex-col gap-3 transition-colors duration-200",
      ...
      <div className="p-4 pb-2 border-b border-border/50 flex items-center justify-between sticky top-0 bg-inherit z-10 rounded-t-xl backdrop-blur-sm">
         <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-foreground/80 uppercase tracking-tight">{title}</h3>
         </div>
            ...
            <AnimatePresence mode="popLayout">
                {tasks.map((task) => (
                    ...
                    >
                        ...
                        onMovePrev={onMoveStatus && !isFirst ? () => onMoveStatus(task.id, 'prev') : undefined}
                        onMoveNext={onMoveStatus && !isLast ? () => onMoveStatus(task.id, 'next') : undefined}
...
function KanbanSkeleton({ isMobile }: { isMobile: boolean }) {
    return (
        <div className={cn("flex h-full gap-4 pb-4 overflow-hidden", isMobile ? "flex-col" : "")}>
            {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex-1 min-w-[300px] bg-muted/30 rounded-xl p-4 flex flex-col gap-3">
