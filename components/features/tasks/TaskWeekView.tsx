...
export function TaskWeekView({
  ...
  const handleNextView = () =>
    setViewOffset((prev) => Math.min(days.length - visibleDaysCount, prev + 1));
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (columns[targetDateStr] !== undefined) {
      const task = tasks.find((t) => t.id === taskId);
      if (task) {
        const currentTaskDate = task.dueDate
          ? startOfDay(parseISO(task.dueDate)).toISOString()
          : null;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 px-1 gap-4">
        <div className="flex items-center gap-2 bg-muted/40 p-1 rounded-lg">
          ...
          <span className="text-sm font-medium w-32 text-center tabular-nums">
            {format(visibleDays[0], "d MMM", { locale: dateLocale })} -{" "}
            {format(visibleDays[visibleDays.length - 1], "d MMM", {
              locale: dateLocale,
            })}
          </span>
          ...
          <Button
            ...
            disabled={
              isMobile ? viewOffset >= days.length - visibleDaysCount : false
            }
            variant="ghost"
            size="sm"
            className="h-8 w-8 p-0"
        ...
        >
          ...
          <span>{t("week.today")}</span>
        </Button>
      </div>

      <div
        className={cn(
          "grid gap-4 pb-4 h-full",
...
function WeekColumn({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex flex-col items-center pb-2 border-b border-border/40",
        ...
        >
          {tasks.map((task) => (
            <DraggableTaskCard
              ...
              onMovePrev={
                onMoveDate ? () => onMoveDate(task.id, "prev") : undefined
              }
              onMoveNext={
                onMoveDate ? () => onMoveDate(task.id, "next") : undefined
              }
...
function WeekSkeleton({ visibleDays }: { visibleDays: number }) {
  return (
    <div
      className={`grid gap-4 pb-4 h-full grid-cols-${visibleDays === 2 ? "2" : "7"}`}
    >
      {Array.from({ length: visibleDays }).map((_, i) => (
        <div
          key={i}
          className="rounded-xl p-3 flex flex-col gap-3 h-full bg-muted/20 border border-border/40"
        >
