...
import {
  updateLearningItem,
  updateLearningStructure,
} from "@/actions/lessonUpdating";
import { generateMissingVocabulary } from "@/actions/lessonProcessing";
...
export default function LessonComponentsManager({
  vocabulary,
  structures,
  lessonId,
}: LessonComponentsManagerProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 bg-muted/20 rounded-lg border border-dashed">
        ...
        <h3 className="text-lg font-medium">{t("empty.vocabularyTitle")}</h3>
      </div>
    );
  }
...
function StructuresList({
  ...
  const filteredItems = items.filter((struct) => {
    ...
    const matchesSearch =
      term.length === 0 ||
      struct.sentences.some((s) => s.words.toLowerCase().includes(term));

    const matchesLevel = levelFilter === "all" || struct.level === levelFilter;
    const matchesType = typeFilter === "all" || struct.type === typeFilter;

    return matchesSearch && matchesLevel && matchesType;
  });

  const handleGenerateMissing = async (structure: LearningStructure) => {
    // 1. Identify missing words (no learningItemId)
    // Filter out trivial words (optional, but good practice)
    ...
    structure.sentences.forEach((sent) => {
      sent.order.forEach((w) => {
        if (!w.learningItemId) {
          const cleanWord = w.word.toLowerCase().replace(/[^a-z0-9]/g, "");
          if (cleanWord.length > 2 && !stopwords.has(cleanWord)) {
            missingWords.add(w.word);
          }
        }
      });
    });
    ...
    if (missingWords.size === 0) {
      toast.info(t("structure.noMissingWords"));
      return;
    }
    ...
    try {
      setGeneratingFor(structure.id);
      const result = await generateMissingVocabulary(
        lessonId,
        Array.from(missingWords),
      );

      if (result.success) {
        toast.success(
          t("structure.generationSuccess", { count: result.count ?? 0 }),
        );
        router.refresh(); // Refresh to show new links
      } else {
        toast.error(t("structure.generationError"));
      }
    } catch (error) {
      toast.error(t("structure.generationUnknownError"));
    } finally {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 bg-muted/20 rounded-lg border border-dashed">
        ...
        <h3 className="text-lg font-medium">{t("empty.structuresTitle")}</h3>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {filteredItems.map((struct) => {
          // Check if this structure has missing items to show the button
          const hasMissing = struct.sentences.some((s) =>
            s.order.some(
              (w) =>
                !w.learningItemId &&
                w.word.length > 2 &&
                !["a", "an", "the"].includes(w.word.toLowerCase()),
            ),
          );
                ...
                <div className="flex gap-2">
                  {hasMissing && lessonId && (
                    <Button
                      ...
                      onClick={() => handleGenerateMissing(struct)}
                      disabled={generatingFor === struct.id}
                    >
                      {generatingFor === struct.id ? (
                        <Loader2 className="w-3 h-3 animate-spin mr-2" />
                      ) : (
                        <Wand2 className="w-3 h-3 mr-2" />
                      )}
                      {t("structure.generateMissing")}
                    </Button>
                  )}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {sent.order
                          ...
                          .map((w, wIdx) => (
                            ...
                            >
                              <span
                                className={`font-bold ${w.learningItemId ? "text-blue-700 dark:text-blue-300" : ""}`}
                              >
                                {w.word}
                                {!w.learningItemId &&
                                  w.word.length > 2 &&
                                  !["a", "an", "the"].includes(
                                    ...
                                    <span className="ml-1 text-amber-500 text-[8px]">
                                      ●
                                    </span>
                                  )}
                              </span>
          ...
          <div className="space-y-4">
            ...
            {(isEditing ? formData.meanings : item.meanings)?.map(
              (meaning, idx) => (
                <div
                  key={idx}
                  className="p-4 bg-card/70 rounded border space-y-3"
                >
                    ...
                    <div className="col-span-2">
                      ...
                      {isEditing ? (
                        <Input
                          ...
                          onChange={(e) =>
                            handleMeaningChange(idx, "context", e.target.value)
                          }
                          className="h-8"
                        />
                      ) : (
                  ...
                  <div className="border-t pt-2 mt-2">
                    ...
                    {isEditing ? (
                      <Input
                        ...
                        onChange={(e) =>
                          handleMeaningChange(idx, "example", e.target.value)
                        }
      ...
      <ModalContent className="max-w-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden bg-white">
        <ModalHeader className="px-6 py-4 border-b">
          <ModalTitle className="flex justify-between items-center w-full">
            <span>{t("structure.editTitle", { type: structure.type })}</span>
            {isEditing ? (
              <div className="flex gap-2">
                <Button
                  variant="ghost"
                ...
                <div className="mb-4">
                  ...
                  {isEditing ? (
                    <Input
                      value={sent.words}
                      onChange={(e) =>
                        handleSentenceChange(sIdx, "words", e.target.value)
                      }
                    />
                  ) : (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sent.order
                      ...
                      .map((wordObj, wIdx) => (
                        <div
                          key={wIdx}
                          className={`border p-2 rounded min-w-[100px] flex flex-col gap-1 ${wordObj.learningItemId ? "bg-blue-50 border-blue-200 dark:bg-blue-950/30 dark:border-blue-800" : "bg-accent"}`}
                        >
                          <span
                            className={`font-bold text-center border-b pb-1 ${wordObj.learningItemId ? "text-blue-700 dark:text-blue-300" : ""}`}
                          >
                            {wordObj.word}
                          </span>
                          {isEditing ? (
                            <Select
                              ...
                              onValueChange={(val) =>
                                handleWordRoleChange(
                                  sIdx,
                                  wIdx,
                                  val as GrammaticalRole,
                                )
                              }
                            >
