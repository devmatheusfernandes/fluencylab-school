  ...
  useEffect(() => {
    ...
    const fetchLesson = async () => {
      ...
      if (docSnap.exists()) {
        ...
        setActiveLesson({
          ...data,
          docID: lessonId as string,
          workbook: workbook as string,
        });
      }
    };
      ...
      <AnimatePresence mode="wait">
        {(sidebarOpen || !isMobile) && (
          ...
          >
            ...
            >
              <h2
                className={`font-bold text-lg text-gray-800 dark:text-white ${sidebarCollapsed && !isMobile ? "hidden" : "block"}`}
              >
                Navegação
              </h2>
        ...
        {activeLesson ? (
          <div className="flex-1 p-4 md:p-8 overflow-y-auto overflow-x-hidden bg-background">
            <div className="max-w-4xl mx-auto">
              <LessonViewer
                key={activeLesson.docID ?? ""}
                lesson={activeLesson.docID ?? null}
                workbook={activeLesson.workbook ?? null}
              />
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 p-6 text-center">
            ...
            <button
              onClick={() => setSidebarOpen(true)}
              className="mt-4 md:hidden text-blue-500 underline"
            >
              Abrir menu
            </button>
          </div>
