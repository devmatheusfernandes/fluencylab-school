...
export const QuestionsNodeView: React.FC<any> = ({
  node,
  updateAttributes,
}) => {
  ...
  const renderQuestion = (q: QuestionData) => {
    ...
    return (
      <div
        className={`flex flex-col gap-4 p-5 md:p-6 ${containerClasses} ${statusBorderClass}`}
      >
        ...
        {/* Media (Imagem/Audio) */}
        {q.imageUrl && (
          <div className="relative w-full min-h-[100px] max-h-[300px] overflow-hidden rounded-lg bg-muted/50 border">
            <Image
              src={q.imageUrl}
  ...
  return (
    <NodeViewWrapper className="my-4 max-w-2xl mx-auto">
      {attrs.mode === "question" && attrs.question
        ...
        : null}
      {attrs.mode === "deck" && attrs.deck ? (
        <DeckView deck={attrs.deck} updateAttributes={updateAttributes} />
      ) : null}
    </NodeViewWrapper>
  );
};
...
const DeckView = ({
  ...
  const [deckAnswers, setDeckAnswers] = useState<Record<string, any>>(
    () => deck.useranswers || {},
  );
  const [deckCorrect, setDeckCorrect] = useState<
    Record<string, boolean | null>
  >({});
  const [deckFillValues, setDeckFillValues] = useState<Record<string, string>>(
    {},
  );
  const [collapsed, setCollapsed] = useState<boolean>(() => !!deck.collapsed);

  useEffect(() => {
    let active = true;
    async function load() {
      const ids = Array.isArray(deck.questionIds)
        ? deck.questionIds.filter(Boolean)
        : [];
      if (!ids.length) {
        setDeckQuestions([]);
        return;
  ...
  const setDeckUserAnswer = (q: QuestionData, value: any) => {
    ...
    setDeckAnswers(next);
    updateAttributes?.({ deck: { ...(deck || {}), useranswers: next } });
    if (q.type === "multiple_choice") {
      const opt = Array.isArray(q.options)
        ? q.options.find((o) => o.id === value)
        : null;
  ...
  const q = deckQuestions[currentIndex];
  const qid = q?.id || String(currentIndex);
  const hasFeedback =
    deckCorrect[qid] !== undefined && deckCorrect[qid] !== null;
  ...
  return (
    <div
      className={cn(
        "bg-card border rounded-xl shadow-sm transition-all p-4",
        collapsed && "py-3",
      )}
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <Button
          ...
          onClick={() => {
            ...
            setCollapsed(next);
            updateAttributes?.({ deck: { ...deck, collapsed: next } });
          }}
        >
          ...
          {q ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                  Pergunta {currentIndex + 1} de {deckQuestions.length}
                </span>
                {hasFeedback && (
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                  ...
                  <div className="space-y-3">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setDeckFillValues((prev) => ({ ...prev, [qid]: val }));
                        if (deckCorrect[qid] !== null) {
                          setDeckCorrect((prev) => ({ ...prev, [qid]: null }));
                        }
                      }}
                      placeholder="Digite sua resposta aqui..."
              ...
              <div className="flex items-center justify-between pt-4 border-t mt-4">
                ...
                <div className="text-sm font-medium">
                  {currentIndex + 1} / {deckQuestions.length}
                </div>
                <Button
                  variant="outline"
                  onClick={goNext}
            ...
            <div className="text-sm text-muted-foreground">
              {deckLoading
                ...
                : "Nenhuma pergunta encontrada neste deck."}
            </div>
          )}
        </>
      )}
...
export const QuestionsNode = Node.create({
    ...
    return [
      {
        ...
        getAttrs: (element: HTMLElement) => {
          const mode = element.getAttribute("mode") as
            | "question"
            | "deck"
            | null;
      ...
      },
      {
        ...
        getAttrs: (element: HTMLElement) => {
          const modeAttr = element.getAttribute("data-mode") as
            | "question"
            | "deck"
            | null;
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const questionStr = typeof q === "string" ? q : q ? JSON.stringify(q) : "";
