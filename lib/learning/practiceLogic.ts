import { LearningItem, LearningStructure, Lesson } from "@/types/learning/lesson";
import { PracticeItem, PracticeMode } from "@/types/learning/practice";
import { SRSData } from "@/types/financial/plan";

export function getModeForDay(day: number): PracticeMode {
  switch (day) {
    case 1: return 'flashcard_visual';
    case 2: return 'gap_fill_listening';
    case 3: return 'sentence_unscramble';
    case 4: return 'flashcard_recall';
    case 5: return 'quiz_comprehensive';
    case 6: return 'listening_choice';
    default: return 'review_standard';
  }
}
...
export function generateQuizItems(
  ...
  for (const section of lessonContext.quiz.quiz_sections) {
    for (const question of section.questions) {
        const id = question.relatedLearningItemId || question.relatedLearningStructureId || `quiz-${Math.random().toString(36).substr(2, 9)}`;
        const type: 'item' | 'structure' = question.relatedLearningStructureId ? 'structure' : 'item';
        ...
        if (lessonContext.transcriptSegments && lessonContext.audioUrl) {
           const relatedId = question.relatedLearningItemId || question.relatedLearningStructureId;
           if (relatedId) {
             const segment = lessonContext.transcriptSegments.find(seg => 
                (seg.learningItemIds && seg.learningItemIds.includes(relatedId)) ||
                (seg.learningStructureIds && seg.learningStructureIds.includes(relatedId))
             );
  ...
  switch (mode) {
    ...
    case 'flashcard_recall': // Dia 4 (Visual is hidden in UI based on mode, but data can be same)
      if (isItem) {
        ...
        base.flashcard = {
          front: item.mainText,
          back: item.meanings[0]?.translation || item.meanings[0]?.definition || '',
    ...
    case 'gap_fill_listening': // Dia 2: Ouvir e Escrever
      ...
      if ((segmentIndex === undefined || segmentIndex === -1) && isItem && lessonContext.transcriptSegments) {
          const item = dbItem as LearningItem;
      ...
      if (segmentIndex !== undefined && segmentIndex !== -1 && isItem && lessonContext.transcriptSegments) {
        ...
        const item = dbItem as LearningItem;
        base.gapFill = {
          sentenceWithGap: segment.text.replace(new RegExp(escapeRegExp(item.mainText), 'gi'), "___"),
    ...
    case 'sentence_unscramble': // Dia 3: Estrutura
       if (!isItem) { // Prioritize Structure for Unscramble, but Items can also be unscrambled if they have sentences
         const struct = dbItem as LearningStructure;
         const sentenceData = struct.sentences[0]; // Take first example
         if (sentenceData) {
            const words = sentenceData.order.sort((a,b) => a.order - b.order).map(o => o.word);
            base.unscramble = {
                correctOrder: words,
          ...
          if (item.meanings[0]?.example) {
             ...
             base.flashcard = { front: item.mainText, back: item.meanings[0]?.translation || '' };
          }
       }
       break;
