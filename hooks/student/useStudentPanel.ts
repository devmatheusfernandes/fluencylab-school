      ...
      learningItemsIds:
        lesson.learningItemsIds?.map((item: any) => ({
          ...
          srsData: item.srsData
            ? {
                ...item.srsData,
                dueDate: item.srsData.dueDate
                  ? new Date(item.srsData.dueDate)
                  : undefined,
              }
            : undefined,
        })) || [],
...
export const useStudentPanel = (studentId: string) => {
  ...
  const fetchClasses = useCallback(
    async (month: number, year: number) => {
      ...
      try {
        ...
        // For teachers/admins accessing student data
        const response = await fetch(
          `/api/teacher/students/${studentId}/classes`,
        );
        if (!response.ok) throw new Error("Failed to fetch classes");
  ...
  const addTask = useCallback(
    async (taskText: string, dueDate?: Date) => {
  ...
  const updateClassStatus = useCallback(
    async (classId: string, newStatus: ClassStatus) => {
      try {
        ...
        if (
          ...
          newStatus === ClassStatus.CANCELED_TEACHER
        ) {
  ...
  const updateClassFeedback = useCallback(
    async (classId: string, feedback: string) => {
      try {
        const response = await fetch(`/api/classes/${classId}`, {
          method: "PATCH",
          headers: {
        ...
        setClasses((prev) =>
          prev.map((cls) => (cls.id === classId ? { ...cls, feedback } : cls)),
        );
  ...
  const updateTask = useCallback(
    async (taskId: string, updates: Partial<Task>) => {
      ...
      try {
        ...
        // For teachers/admins accessing student data
        const response = await fetch(
          `/api/teacher/students/${studentId}/tasks/${taskId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
