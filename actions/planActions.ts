...
export async function connectPlanToClasses(planId: string, studentId: string) {
  try {
    ...
    return {
      success: true,
      message: `Plan connected to ${updatesCount} classes successfully`,
      updatesCount,
    };
  } catch (error: any) {
...
export async function assignTemplateToStudent(
  ...
  try {
    ...
    if (
      ...
      (connectResult.updatesCount === 0 && template.lessons.length > 0)
    ) {
      await adminDb.collection("plans").doc(newPlanId).delete();
      throw new Error(
        connectResult.error ||
          "Não foi possível agendar as aulas do template. Verifique se o aluno possui aulas futuras.",
      );
    }
...
export async function createPlan(data: Partial<Plan>) {
  try {
    ...
    if (data.type === "student" && data.studentId) {
      ...
      if (
        !connectResult.success ||
        (connectResult.updatesCount === 0 && planData.lessons.length > 0)
      ) {
        console.warn(
          `[createPlan] Connection failed or no classes scheduled (updates: ${connectResult.updatesCount}). Rolling back plan ${id}.`,
        );
...
export async function updatePlan(id: string, data: Partial<Plan>) {
  try {
    ...
    revalidatePath("/hub/material-manager/plans");
    if (updatedPlan?.studentId)
      revalidatePath(`/hub/manager/users/${updatedPlan.studentId}`);
