...
export class SubscriptionService {
  ...
  async createSubscription(params: CreateSubscriptionParams) {
    ...
    if (
      ...
      userRole !== UserRoles.GUARDED_STUDENT
    ) {
  ...
  async cancelSubscription(params: CancelSubscriptionParams) {
    ...
    if (cancellationFee > 0) {
      await this.generateCancellationFeePayment(
        subscription.userId,
        cancellationFee,
        subscription.id,
      );
   ...
   */
  private async getPixPaymentStatus(
    ...
    const cancellationFeePayment = payments.find(
      (p) => p.type === "cancellation_fee" && p.status !== "paid",
    );
    ...
    if (!currentPayment) {
      currentPayment = payments.find(
        (p) => p.status === "pending" && p.type !== "cancellation_fee",
      );
    }
    ...
    if (subscription.status === "canceled") {
      ...
      if (
        ...
        currentPayment.type !== "cancellation_fee"
      ) {
    ...
    const status: PaymentStatus = {
      ...
      subscriptionStatus,
      nextPaymentDue: currentPayment
        ? new Date(currentPayment.dueDate)
        : undefined,
      lastPaymentDate: latestPaidPayment?.paidAt
    ...
    if (
      currentPayment &&
      (currentPayment.status === "available" || currentPayment.pixCode)
    ) {
      status.pixCode = currentPayment.pixCode;
      status.pixQrCode = currentPayment.pixQrCode;
      status.pixExpiresAt = currentPayment.pixExpiresAt
        ? new Date(currentPayment.pixExpiresAt)
        : undefined;
    }
  ...
  private async handleProviderPaymentPaid(providerPaymentId: string) {
    ...
    try {
      const subscription = await this.getSubscriptionPrivate(
   ...
   */
  private async handlePixPaymentSuccess(
    subscriptionId: string,
    userId: string,
  ) {
    const subscription = await this.getSubscriptionPrivate(subscriptionId);
    ...
    await this.updateSubscription(subscriptionId, {
      ...
      status:
        newPaymentsCompleted >= subscription.totalPayments
          ? "canceled"
          : "active", // Complete contract
    });
    ...
    if (newPaymentsCompleted < subscription.totalPayments) {
      ...
      const nextPayment = payments.find(
        (p) => p.paymentNumber === newPaymentsCompleted + 1,
      );
      ...
      if (nextPayment) {
        ...
        if (now >= twoDaysBeforeDue) {
          await this.makePaymentAvailable(
            subscriptionId,
    ...
    console.log(
      `PIX payment successful for user ${userId}, subscription ${subscriptionId}, payment ${newPaymentsCompleted}/${subscription.totalPayments}`,
    );
  }

  private shouldChargeCancellationFee(
    subscription: MonthlySubscription,
  ): boolean {
  ...
  private async getSubscriptionPrivate(
    id: string,
  ): Promise<MonthlySubscription | null> {
    const doc = await adminDb.collection("subscriptions").doc(id).get();
    if (!doc.exists) return null;
  ...
  private async getPendingPixPayment(
    subscriptionId: string,
  ...
  private async generateCancellationFeePayment(
    userId: string,
    feeAmount: number,
    subscriptionId: string,
  ) {
  ...
  async getSubscriptionOverview(
    ...
    const pendingPayments = payments.filter((p) =>
      ["pending", "available"].includes(p.status),
    );
    ...
    return {
      ...
      contractProgress: {
        ...
        percentage: Math.round(
          (subscription.paymentsCompleted / subscription.totalPayments) * 100,
        ),
      },
    };
  }
