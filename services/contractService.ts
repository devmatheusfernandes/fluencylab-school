...
export class ContractService {
  ...
  async createContract(
    ...
    try {
      ...
      const existingStatus = await contractRepository.getContractStatus(
        studentId
      );
      if (
  ...
  async adminSignContract(
    ...
    try {
      ...
      const updatedStatus = await contractRepository.getContractStatus(
        studentId
      );
  ...
  async processContractRenewals(): Promise<ContractRenewalJob[]> {
    try {
      ...
      const guardedStudentUsers = await userAdminRepository.findUsersByRole(
        "guarded_student"
      );
      ...
      const materialManagerUsers = await userAdminRepository.findUsersByRole(
        "material_manager"
      );
