...
import { 
  Clock, BookOpen, ChevronRight, CheckCircle, Lock, 
  PlayCircle, ArrowLeft, Globe, LayoutList, GraduationCap 
} from "lucide-react";
...
export default function CourseDetailPageContent() {
  ...
  const fetchCourseDetails = async () => {
    ...
    try {
      ...
      if (enrollmentData) {
        const completedLessons = Object.values(enrollmentData.progress || {}).filter(Boolean).length;
            ...
            <div className="p-6 flex flex-col justify-between flex-1 gap-4">
              ...
              <div className="space-y-4">
                 ...
                 <div className="flex flex-wrap gap-2">
                    <Badge variant="secondary" className="gap-1 font-normal"><Globe className="w-3 h-3" /> {course.language}</Badge>
                    <Badge variant="secondary" className="gap-1 font-normal"><Clock className="w-3 h-3" /> {course.duration}</Badge>
                    <Badge variant="secondary" className="gap-1 font-normal"><LayoutList className="w-3 h-3" /> {course.totalLessons} lições</Badge>
                 </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <span className={`text-sm font-medium truncate ${completed ? 'text-muted-foreground line-through decoration-emerald-500/50' : ''}`}>
                                                    {lesson.title}
                                                </span>
                                            </div>
