...
export async function POST(request: NextRequest) {
  try {
    ...
    let callId: string | undefined = body?.callId;

    if (!studentId || typeof studentId !== 'string' || !studentId.trim()) {
      return NextResponse.json({ error: 'studentId inválido.' }, { status: 400 });
    }

    // Gera o callId no servidor se não tiver sido enviado
    if (!callId) {
      const participants = [session.user.id, studentId].sort();
      callId = participants.join('-');
    }

    // Atualiza o campo callId do aluno
    ...
    await studentRef.update({ callId });

    return NextResponse.json({ studentId, callId });
  } catch (error) {
